...
window.addEventListener("scroll", () => {
  ...
  // Smooth movement for saluteText and saluteImg on scroll
  saluteTextContainer.style.transform = `translate(${offsetY * -0.8}px, ${offsetY * -0.6}px)`;
  saluteImgContainer.style.transform = `translate(${offsetY * -0.6}px, ${offsetY * 0.6}px)`;

  // Background movement for job title containers
  jobTitleContainers[0].style.backgroundPositionY = `${offsetY * 0.5}px`;

  // Adjust jobTitles scroll effect for smooth entry from the left
  jobTitles[0].style.transform = `translateX(calc(${Math.max(0, -100 + offsetY * 0.8)}px))`;
});
